...
func calcHawkAuth(ctx *cli.FFSContext, session HawkSession, requestMethod string, requestURI string, body string, contentType string) (string, error) {
	...
	hmacBuilder := hmac.New(sha256.New, []byte(session.HawkKey))
	hmacBuilder.Write([]byte(sigstr))
	mac := base64.StdEncoding.EncodeToString(hmacBuilder.Sum(nil))
